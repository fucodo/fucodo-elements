  ...
  var PackagistList = class extends HTMLElement {
    ...
    constructor() {
      ...
      this._state = {
        ...
        perPage: 15,
        sort: "downloads",
        // downloads | favers | (fallback: relevance)
        loading: false,
        error: null,
        total: 0,
        results: []
    ...
    `;
      const status = loading ? `<div class="alert alert-info py-2 mb-3" role="status">Lade Pakete von Packagist\u2026</div>` : error ? `<div class="alert alert-danger py-2 mb-3">Fehler: ${this._escape(error)}</div>` : `<div class="small text-muted mb-2">${total} Treffer \xB7 Seite ${page} von ${Math.max(1, Math.ceil(total / perPage))}</div>`;
      const list = results.map((item) => this._renderItem(item)).join("") || (!loading && !error ? `<div class="alert alert-warning py-2">Keine Pakete gefunden.</div>` : "");
      ...
      <nav aria-label="Packagist pagination">
        <ul class="pagination mb-0">
          <li class="page-item ${page <= 1 ? "disabled" : ""}">
            <button class="page-link" data-pl-action="prev" ${page <= 1 ? 'tabindex="-1" aria-disabled="true"' : ""}>Zur\xFCck</button>
          </li>
