(() => {
  // packages/fucodo-form-number/index.js
  var CurrencyInput = class extends HTMLElement {
    constructor() {
      super();
    ...
    _currencySymbol(code) {
      const map = { EUR: "\u20AC", USD: "$", GBP: "\xA3", CHF: "CHF" };
    ...
    _parseToCents(str) {
      ...
      if (sepIndex !== -1) {
        ...
        const euros2 = parseInt(intDigits || "0", 10) || 0;
        let decimals = 0;
        if (fracDigits.length === 0) {
          decimals = 0;
        } else if (fracDigits.length === 1) {
          decimals = parseInt(fracDigits, 10) * 10;
        } else {
          decimals = parseInt(fracDigits.slice(0, 2), 10);
        }
        return sign * (euros2 * 100 + decimals);
      }
