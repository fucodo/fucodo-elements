...
// responsive-table.js
// Web Component: <responsive-table> ... <table> ... </table> </responsive-table>
// - No Shadow DOM
// - Adds td[data-label] from thead th
// - Injects scoped CSS
// - Auto-computes min width for desktop mode unless min-width attribute is set
// - Adds table classes: rt-mobile / rt-desktop

const STYLE_ID = "responsive-table-wc-styles";
const MOBILE_CLASS = "rt-mobile";
const DESKTOP_CLASS = "rt-desktop";
...
function extractHeaderLabels(table) {
    // Prefer thead -> first row of thead
    let headerRow =
        table.tHead?.rows?.[0] ||
    ...
    // Expand colSpans so td index mapping works for simple tables
...
function applyDataLabels(table) {
    ...
    // Apply to TBODY and TFOOT if present
    const bodies = [];
    ...
    for (const body of bodies) {
        for (const row of Array.from(body.rows || [])) {
            ...
            for (const cell of Array.from(row.cells || [])) {
                // Skip THs in body rows (row headers), but still advance index
...
function measureNaturalTableWidth(table) {
    // Clone into an offscreen container to measure "desktop" width without layout constraints
...
class ResponsiveTable extends HTMLElement {
    ...
    connectedCallback() {
        ...
        // Watch table content changes (e.g., AJAX-rendered rows)
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        if (name === "min-width" && oldValue !== newValue) {
            this._autoMinWidth = null; // reset auto
            this._ensureMinWidth();
            this._updateMode();
        }
    }
    ...
    _scheduleRefresh() {
        ...
        queueMicrotask(() => {
            ...
            applyDataLabels(this._table);
            // Recompute auto min width only if user didn't set one
