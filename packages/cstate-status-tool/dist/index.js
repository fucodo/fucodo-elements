(() => {
  ...
  // packages/cstate-status-tool/c-state-build-service.js
  var CStateBuildService = class {
    ...
    static _validate(titleRaw, dateTime, messageDetails, incidentType, affectedSystemsNodes) {
      ...
      if (incidentType === null || incidentType === void 0) {
      ...
      for (let line of messageLines) {
        if (line.trim().startsWith("# ")) {
          throw new Error('Meldungstext/Beschreibung:\nBitte verwenden Sie nur Markdown-\xDCberschriften ## (H2) oder tiefer.\nH1-\xDCberschriften ("# ") sind reserviert und sollten nicht verwendet werden.');
        }
      }
    }
...
`;
  ...
  // packages/cstate-status-tool/index.js
  var import_lit_element = __require("https://esm.sh/lit-element");
  var import_decorators = __require("https://esm.sh/lit@3.2.1/decorators.js");
  var CstateStatusTool = class extends import_lit_element.LitElement {
    static properties = {
      base: { type: String },
      // c-state index.json URL
      defaultDetails: { type: String },
      // Markdown-Template für Beschreibung
    ...
    async _loadSystemsFromCstate() {
      ...
      try {
        ...
        if (!Array.isArray(json.systems)) {
          throw new Error('index.json enth\xE4lt kein "systems"-Array');
        }
        this.systems = json.systems.filter((s) => typeof s?.name === "string" && s.name.trim() !== "").map((s, idx) => ({
          id: idx,
          name: s.name,
          label: s.name
        }));
        ...
        this._systemsError = "Die Liste der Dienste konnte nicht von der Statusseite geladen werden. Bitte pr\xFCfen Sie den Basis-URI oder versuchen Sie es sp\xE4ter erneut.";
        this._systemsLoading = false;
      }
    }
...
_${signature}_`;
    ...
    _onSubmit(e) {
      ...
      const payload = {
        ...
        filename
        // fertiger Dateiname für c-state
      };
      this.dispatchEvent(
    ...
    renderSystemsFieldset() {
      if (this._systemsLoading) {
        return import_lit_element.html`<div class="form-text">
        Dienste werden von der Statusseite geladen…
      </div>`;
      }
      if (this._systemsError) {
        return import_lit_element.html`
      ...
      <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4">
        ${this.systems.map(
        (system) => import_lit_element.html`
            <div class="col mb-2">
